...
void main() {
  ...
  // return;
     // Create noise at different scales and movement speeds
        float scale1 = 0.4; // Larger scale for main noise pattern
        float scale2 = 0.6; // Smaller scale for detail
        
        float scaleX = resolutionX/1920.0;
        // Adjust X scale to be wider
        vec2 scale1XY = vec2(scale1 * 9.0*scaleX, scale1/1.0);
        vec2 scale2XY = vec2(scale2 * 2.0*scaleX, scale2/1.0);
